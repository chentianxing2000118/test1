...
public class StringUtil {
	...
	public static boolean hasLength(String src) throws UnsupportedEncodingException{
		return null!=src && src.length()>0;
	}
	
	//方法2：判断源字符串是否有值，空引号(空白字符串)和空格也算没值 (5分)
	public static boolean hasText(String src){
		return null!=src && src.length()>0 && src.trim().length()>0;
	}
	...
	public static String randomChineseString(int length){
		...
		for (int i = 0; i < length; i++) {
			...
			try {
				if(str2.equals(new String(str2.getBytes("GB2312"),"GB2312"))) {
					str += str2;
				}else {
					i--;
				}
	...
	public static String generateChineseName(){
		...
		if(r.nextBoolean()) {
			str += randomChineseString(1);
		}else {
			str += randomChineseString(2);
		}
		return str;
	}
